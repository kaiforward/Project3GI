$(document).ready(function() {
	...
	// TOGGLES VISIBILITY OF LINKBAR WHEN NAVBUTTON IS CLICKED
	$(".navbutton").on("tap click", function(){ // if navbutton clicked
	...
	$("#pagewrapper").on("tap click", function(){ // if anywhere but navbar is clicked.
		if($('#ismobile').is(':visible')) { // if on mobile
			if ($('#linkbar').hasClass('mobilevisible')) { // and the menu has been opened
	...
	// generate random background image each time new page is visited.
	letter = Math.floor(Math.random() * 7+1); // i dont like javascript random	
	// using template literals to insert random variable into background img src.
	// `` the weird commas around the string allow the ${} to work, put variable inside.
	$('#backgrounddiv').prepend(`<img src="/static/images/background${letter}.jpg"class="abs backgroundimg">`)				
});
